...
class TestCitiesAround(TestCase):
    ...
    def test_cities_around_success(self, mock_get_cities_around):
        ...
        response = self.client.get(f"/cities-around?city={city}&distance_km={distance_km}")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), {"city": city, "distance_km": distance_km, "cities_around": expected_result})
        mock_get_cities_around.assert_called_once_with(city, distance_km)
